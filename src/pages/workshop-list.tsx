...
import {
    WORKSHOPS_SIMPLE_QUERY,
    STATES_QUERY,
    INSURANCE_COMPANIES_QUERY,
    ORGANIZATION_BY_ID_QUERY,
} from "@/lib/sektor-api/queries";
import { Query, SupplierType, StateType, InsuranceCompanyType, SocialMediaPlatform, ServiceSupplierTypes } from "@/lib/sektor-api/__generated__/types";
...
const WorkshopList = () => {
    ...
    const [contactModalWorkshop, setContactModalWorkshop] = useState<SupplierType | null>(null);

    const toggleSegurosAccordion = (workshopId: string) => {
    ...
    const { data: workshopsData, loading: isLoadingWorkshops } = useQuery<Query>(WORKSHOPS_SIMPLE_QUERY, {
        variables: {
            pagination: { offset: 0, limit: 10000 },
            filter: {
                serviceType: ServiceSupplierTypes.Workshop,
    ...
    const allWorkshops = workshopsData?.publicSuppliers?.items || [];
    ...
    const workshops = allWorkshops.filter((workshop) => {
        const stateMatch =
            !selectedState ||
            workshop.offices?.some((office) => office.address?.state?.id === Number(selectedState));
        const insuranceMatch =
            !selectedInsurance ||
            workshop.insuranceCompanyRelations?.some((r) => r.insuranceCompanyId === selectedInsurance);
    ...
    const totalPages = Math.ceil(workshops.length / CARDS_PER_PAGE) || 1;
                ...
                <div className="w-full">
                    {isLoadingWorkshops ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
                            {paginatedWorkshops.map((workshop: SupplierType) => {
                                const offices = workshop.offices || [];
                                ...
                                const whatsappLink = workshop.socialMediaLinks?.find((l) => l.platform === SocialMediaPlatform.Whatsapp);
                                const whatsappUrl = whatsappLink?.url
                                    ? `https://wa.me/${whatsappLink.url.replace(/\D/g, "")}`
                                    : "";
                                const websiteRaw = workshop.socialMediaLinks?.find((l) => l.platform === SocialMediaPlatform.Website)?.url?.trim() ?? "";
                                ...
                                const googleMapsUrl = displayAddress
                                    ...
                                    : "";
                                const relations = workshop.insuranceCompanyRelations || [];
    const insuranceCompanies = workshop.insuranceCompanies || [];
      ...
      insuranceCompanies.length > 0
        ...
        : relations.map((rel) => ({
            ...
            relation: rel,
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="p-4 flex flex-col gap-3 flex-1 min-h-0">
                                                    ...
                                                    {insuranceListForBadges.length > 0 && (
                                                        <div className="mt-1.5">
                                                            <div className="flex flex-wrap gap-1.5">
                                                                ...
                                                                {(segurosAccordionOpenIds.has(workshop.id)
                                                            ...
                                                            {insuranceListForBadges.length > SEGUROS_VISIBLE_SIN_EXPANDIR && (
                                                                <button
                                                                    type="button"
                                                                    onClick={() => toggleSegurosAccordion(workshop.id)}
                                                                ...
                                                                >
                                                                    ...
                                                                    <FontAwesomeIcon
                                                                        icon={segurosAccordionOpenIds.has(workshop.id) ? faChevronUp : faChevronDown}
                                                                        className="text-[10px]"
                                                                    />
                                                                </button>
                                                            )}
                                            ...
                                            {/* Solo se muestran los botones para los que el taller tiene dato */}
                                            <div className="flex flex-wrap gap-1.5">
                                                {phone && (
                                                    <a
                                                        href={`tel:${phone}`}
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => setContactModalWorkshop(workshop)}
                    ...
                    {!isLoadingWorkshops && workshops.length > CARDS_PER_PAGE && (
            ...
            <Modal
                footer={null}
                open={!!contactModalWorkshop}
                onCancel={() => setContactModalWorkshop(null)}
                className="!max-w-md !w-[calc(100%-2rem)]"
                centered
                title={null}
                closeIcon={
                ...
                styles={{ header: { borderBottom: "none", paddingBottom: 0 } }}
            >
                {contactModalWorkshop && (() => {
                    const firstOffice = contactModalWorkshop.offices?.[0];
                    ...
                    const email = (contactModalWorkshop as SupplierType & { email?: string | null }).email ?? undefined;
                    const socialLinks = contactModalWorkshop.socialMediaLinks ?? [];
